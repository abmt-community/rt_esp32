...
void model_adapter_std::set_model(abmt::rt::model* m){
...
void model_adapter_std::on_hello(uint16_t version){
...
void model_adapter_std::on_request_signal_def(){
...
void model_adapter_std::on_request_parameter_def(){
...
void model_adapter_std::on_set_daq_list(abmt::blob& lst){
...
void model_adapter_std::on_set_paq_list(abmt::blob& lst){
	paq_list.clear();
...
void model_adapter_std::on_set_parameter(abmt::blob& data){
	if(data.size < 8){
		return;
	}
...
void model_adapter_std::on_command(string cmd){
	if(cmd == "quit"){
		connected = false;
		clear_daq_lists();
		on_quit();
...
void model_adapter_std::save_parameters(){
	on_save_parameters();
}

void model_adapter_std::clear_daq_lists(){
	for(size_t i = 0; i < mdl->rasters.length; ++i){
		daq_lists[i].clear();
	}
}

void model_adapter_std::send_daq(size_t raster){
...
void model_adapter_std::send_all_parameters(){
	size_t size_to_send = 0;
...
model_adapter_std::~model_adapter_std(){
	if(daq_lists != 0){
		delete[] daq_lists;
	}
};
